...
export default class kategoriedit{
   ...
   renderedit() {
    const transnoHider = $("#transnoHider").val();
        //  console.log(transnoHider); return;
    $.ajax({
        ...
        headers: { 'url': 'mskat/listkategorifinalbyid' },
...
render() {
 ...
 setKategori(result, isEdit = false) {
    ...
    result.forEach(element => {
        ...
        html += `
            <div class="row mb-2">
                <label for="total_kategori${ktg}" style="width: 20%;" class="col-form-label">
                    Kategori ${ktg}
                </label>
                <div style="width: 35%;" class="col-sm-6">
                    <span id="total_kategori${ktg}" style="display: flex; justify-content: flex-end;">
                        ${totalHTML}
                    </span>
                </div>
            </div>`;
