...
class FPDF_AutoWrapTable extends FPDF {
    ...
    public function rptDetailData() {
        ...
        //header
			...
			$this->Cell(180 ,10,'PACKING LIST',0,1,'C');//end of line
            ...
            $this->Cell(189 ,3,'We hereby confirm  you to supply the commodity on terms and conditions as specfiied below:',0,1);
          ...
          $this->ln();
            ...
            $this->SetX($left +=15); $this->Cell(55,$h,'DESCRIPTION',1, 0,'C');
            $this->SetX($left +=55); $this->Cell(18,$h,'QTY',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(10,$h,'UNIT',1, 0,'C');
            $this->SetX($left +=10); $this->Cell(18,$h,'QTY(CTNS)',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'GW(KGS)',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'NW(KGS)',1,0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'MEAS(CBM)',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'KOMPOSISI %',1, 1,'R');
            $this->SetFont('Arial','',7);
            //$this->SetDrawColor(255,255,255); ini untuk mengilangkan gari di row table
            $this->SetWidths(array(8,15,55,18,10,18,18,18,18,18));
            $this->SetAligns(array('C','L','L','R','C','R','R','R','R','R'));
            ...
            foreach($detail as $baris){
             ...
             $this->Row(
                ...
                $rep_CTNS = str_replace(",","",$baris['CTNS']);
                $rep_GTNS = str_replace(",","",$baris['GTNS']);
                $rep_KGS = str_replace(",","",$baris['KGS']);
                $rep_CBM = str_replace(",","",$baris['CBM']);

              
                $total_qty +=(float)$rep_qty;
                $total_ctns +=(float)$rep_CTNS;
                $total_gtns +=(float)$rep_GTNS;
                $total_kgs +=(float)$rep_KGS;
                $total_cmb +=(float)$rep_CBM;
                $total_komp +=(float)$baris['Komposisi_jm'];
            }

            $label = "T O T A L  ";
            $set_qty = $this->getformatnumer($total_qty);
            $set_ctns = $this->getformatnumerqty($total_ctns);
            $set_gtns = $this->getformatnumer($total_gtns);
            $set_kgs = $this->getformatnumer($total_kgs);
            $set_cmb = $this->getformatnumer($total_cmb);
            ...
            $this->SetX($left +=78); $this->Cell(18,$h,$set_qty,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(10,$h,'',1, 0,'R');
            $this->SetX($left +=10); $this->Cell(18,$h,$set_ctns,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_gtns,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_kgs,1,0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_cmb,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_komp,1, 1,'R');
            $this->Cell(0,0, '', 0, 1,'C');
            $y = $this->GetY();
			$x = $this->GetX();
            $width =40;
