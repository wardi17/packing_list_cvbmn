...
class FPDF_AutoWrapTable extends FPDF {
	...
	$this->Cell(0,2, '', 0, 1,'C');
          ...
          $this->ln();
            ...
            $this->SetX($left +=18); $this->Cell(18,$h,'AMOUNT PIB',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'HPP AWAL',1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,'HPP AKHIR',1, 1,'R');
            $this->SetFont('Arial','',7);
            //$this->SetDrawColor(255,255,255); ini untuk mengilangkan gari di row table
            $this->SetWidths(array(8,15,55,18,20,18,18,18,18));
            ...
            foreach($detail as $baris){
             ...
             $this->Row(
                ...
                $rep_awal = str_replace(",","",$baris['hpp_awal']);
                $rep_akhir = str_replace(",","",$baris['hpp_akhir']);
              
                $total_qty +=(float)$rep_qty;
                $total_biayapib +=(float)$rep_biayapib;
                $total_amount +=(float)$rep_Amount;
                $total_komp +=(float)$baris['Komposisi_jm'];
                $total_hppawal +=(float)$rep_awal;
                $total_hppakhir +=(float)$rep_akhir;
            }
            ...
            $set_komp = $this->getformatnumer_kom($total_komp);
             ...
             $set_hppawal = $this->getformatnumerqty($total_hppawal);
             $set_hppakhir = $this->getformatnumerqty($total_hppakhir);
            ...
            $this->SetX($left +=18); $this->Cell(18,$h,$set_amount,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_hppawal,1, 0,'R');
            $this->SetX($left +=18); $this->Cell(18,$h,$set_hppakhir,1, 1,'R');
            $this->Cell(0,0, '', 0, 1,'C');
            $y = $this->GetY();
			$x = $this->GetX();
            $width =40;
