...
SELECT *
FROM fn_my_permissions('POPAKINGLIST_KURS_HISTORY', 'OBJECT')
WHERE permission_name = 'DROP';

SELECT *
FROM INFORMATION_SCHEMA.TABLES
WHERE TABLE_NAME LIKE '%POPAKINGLIST_KURS_HISTORY%'

DROP TABLE POPAKINGLIST_KURS_HISTORY
DROP TABLE FrowaderDetail_HISTORY
DROP TABLE  POPAKINGLIST_KURSDETAIL_HISTORY
...
CREATE TABLE [dbo].[POPAKINGLIST_KURS_HISTORY](
...
ALTER TABLE [dbo].[POPAKINGLIST_KURS_HISTORY]
...
CREATE TABLE FrowaderDetail_HISTORY(
	...
	FOREIGN KEY (No_Pls) REFERENCES POPAKINGLIST_KURS_HISTORY(No_Pls),
);


 CREATE TABLE POPAKINGLIST_KURSDETAIL_HISTORY(
	[No_Pls] [char](20) NOT NULL,
	[ItemNo] [float] NULL,
